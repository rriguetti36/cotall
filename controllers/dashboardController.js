...
exports.getDashboard = async (req, res) => {
  try {
    console.log("entra a la consulta getDashboard");
...
exports.asesorPie = async (req, res) => {
  try {
    console.log("entra a la consulta asesorPie");

    const asesores = await dashboard.asesores(res.locals.idcia);
    if (!asesores || asesores.length === 0) {
      return res.status(404).json({ message: 'No se encontraron asesores para esta compañía' });
    }
